classdef SimpleQueue < handle
    %SIMPLEQUEUE Simple implementation of a Queue data structure.
    %   A very simple queue data structure (FIFO).
    %
    % see also SimpleNodes

    properties (Access = private)
        firstNode (:,1) SimpleNode {mustBeScalarOrEmpty} % start of queue
        lastNode (:,1) SimpleNode {mustBeScalarOrEmpty} % end of queue
    end
    ...
    methods
        function obj = SimpleQueue()
            % SIMPLEQUEUE construct a new empty queue.
            %
            % Outputs:
            % * obj: A new empty SimpleQueue.
            obj.numel = 0;
        end
        ...
        function push(obj,data)
            ...
            %
            % Inputs:
            % * obj: SimpleQueue to add data to.
        ...
        function data = peak(obj)
            ...
            arguments
                obj (1,1) SimpleQueue {mustBeNonEmptyQueue}
            end
        ...
        function data = pop(obj)
            ...
            if obj.numel == 1 % special case when we only have one more element
                obj.firstNode = SimpleNode.empty(0,1);
                obj.lastNode = SimpleNode.empty(0,1);
            else
        ...
        function array = toCell(obj)
            ...
            %
            % Outputs:
            % * aray: An nx1 cell array with all the elements from the
            %   queue.
            arguments
                obj (1,1) SimpleQueue
            end
