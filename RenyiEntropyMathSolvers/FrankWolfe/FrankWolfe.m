classdef FrankWolfe
    %FRANKWOLFE Collection of Frank Wolfe algorithm varients.
    % A collection of Frank Wolfe algorithm varients for minimizing convex
    % optimization problems over the field R. Each varient uses the same
    ...
    % field R represented with nd arrays, and whose inner product can be
    % computed with the function FrankWolfe.inProdR.
    %
    % Each varient uses the following base method signature:
    %
    %% Input:
    %
    ...
    %   The function subProblem has the signature: Input:
    ...
    %% Options:
    %
    % Options are represented through name value pair arguments at the end
    % of the function call. For example, in the arguments block, add
    %   options.option1 (1,1) double options.option2 (:,1) string
    ...
    %   more details.
    % * output: Struct containing a small amount of aditional information
    ...
    methods (Static)
        function [optPoint, optVal, FWExitFlag,output] = vanilla(initialPoint,func,...
            ...
            %   real number.
            % * storePoints (false): Store each step point in a SimpleQueue
            %   data structure under "pointsQueue" in the debugInfo.
            %
            % Output:
            ...
            % * output: Struct containing a small amount of aditional
            ...
            %
            % DebugInfo:
            % * subproblemStatusMessage: The message returned from call to
            %   subproblem.
            % * pointsQueue: Only present when storePoints is true. A
            %   SimpleQueue with each step point.
            %
            % see also FrankWolfe fminbnd optimset SimpleQueue
            arguments
                initialPoint double
            ...
            if options.storePoints
                pointsQueue = SimpleQueue();
                debugInfo.storeInfo("pointsQueue",pointsQueue)
                pointsQueue.push(optPoint);
                gapsQueue = SimpleQueue();
                debugInfo.storeInfo("gapsQueue",gapsQueue)
                fValQueue = SimpleQueue();
                debugInfo.storeInfo("fValQueue",fValQueue)
                stepQueue = SimpleQueue();
                debugInfo.storeInfo("stepQueue",stepQueue)
                gradQueue = SimpleQueue();
                debugInfo.storeInfo("gradQueue",gradQueue)
            end
            ...
            % print formating for loop ASCII table
            ...
            for iter = 1:options.maxIter
                ...
                if options.storePoints; gapsQueue.push(gap); fValQueue.push(func(optPoint)); stepQueue.push(deltaPoint+optPoint); gradQueue.push(gradf); end

                %get the Frank Wolfe lower bound at this point
                tempFWLB = optVal - abs(gap); %negative gaps from instability must be handled
        ...
        function [optPoint, optVal, FWExitFlag, output] = pairwise(initialPoint,func,gradFunc,subProblem,debugInfo,printInfo,options)
            ...
            activeIndexes = 1; % list of indeces for points that are still active.
