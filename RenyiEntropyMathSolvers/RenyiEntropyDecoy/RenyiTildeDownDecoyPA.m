classdef RenyiTildeDownDecoyPA
    ...
    methods (Static)
        function cost = privacyAmpUniqueAcceptance(choiEveAPrimeB,testStateAAPrime,...
                genStateAAPrime,keyProj,krausOps,testProb,alpha,dimAPrime)
            % This is broken
            arguments
            ...
            penaltyTerm = penaltyTermUniqueDecoy(testStateAAPrime,...
                ChoiEveAPrimeB,dimA,testCons,testProb);

            rhoAB = PartialMap(genStateAAPrime,choiEveAPrimeB,2,[dimA,dimAPrime]);
            cost = penaltyTerm/(alpha-1) + (1-testProb)...
                *RenyiTildeDown.conEnt(rhoAB,keyProj,krausOps,alpha);
        end
