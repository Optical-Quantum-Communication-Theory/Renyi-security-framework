function qkdInput = RenyiPhaseImpDecoy46LossyPreset()
...
qkdInput.addFixedParameter("NA",23);%20

%Block cutoff used for key rate starts from 0,1,...
qkdInput.addFixedParameter("blockCutoff",8); %8
...
%on Bob's side
descriptionModule = QKDDescriptionModule(@RenyiPhaseImpDecoy46LossyDescriptionFunc);

qkdInput.setDescriptionModule(descriptionModule);

% channel model (Channel stays the same with phase imperfections)
channelModule = QKDChannelModule(@RenyiDecoy46LossyChannelFunc);
qkdInput.setChannelModule(channelModule);

% key rate function
keyModule = QKDKeyRateModule(@RenyiPhaseImpDecoy46KeyRateFunc);
...
mathSolverMod = QKDMathSolverModule(@renyiChoiBlockFrankWolfeSolver,mathSolverOptions);
