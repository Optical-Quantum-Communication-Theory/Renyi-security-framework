function [keyRate, modParser] = RenyiDecoy46KeyRateFunc(params,options,mathSolverFunc,debugInfo)
...
%creates operators and bounds for squashing constraints
function [squashingOpsTest,squashingOpsGen,lowerBndSubspaceTest,lowerBndSubspaceGen] = ...
    createSquashingCons2(POVMATest,POVMAGen,probSignalConTest,probSignalConGen,subspaceBnd,BobSubspaceProj,BobMultiPOVM,NB)
    %create operators for squashing constraints in Gen and Test rounds (both
    %can be used)
    squashingOpsGen = cell(1,numel(POVMAGen));
    squashingOpsTest = cell(1,numel(POVMATest));
    
    BobSquashingOp = BobMultiPOVM + subspaceBnd*BobSubspaceProj;
    
    for indexA = 1:numel(POVMAGen)
        squashingOpsGen{indexA} = kron(POVMAGen{indexA},BobSquashingOp);
    end
    
    for indexA = 1:numel(POVMATest)
        squashingOpsTest{indexA} = kron(POVMATest{indexA},BobSquashingOp);
    end
    
    %lower bound on suspace multiplied by probability of Alice sending that
    %state ordered gen,test (transposed to have same shaped as the squashing
    %operators)
    lowerBndSubspaceGen  = (subspaceBnd *probSignalConGen).';
    lowerBndSubspaceTest = (subspaceBnd *probSignalConTest).';
end
