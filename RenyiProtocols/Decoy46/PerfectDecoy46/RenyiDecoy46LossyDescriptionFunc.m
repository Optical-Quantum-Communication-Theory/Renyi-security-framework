function [newParams,modParser] = RenyiDecoy46LossyDescriptionFunc(params, options, debugInfo)
...
GammaH = blkdiag(pzB*diag([0,1,0]), diag(zket(7,1)));
GammaV = blkdiag(pzB*diag([0,0,1]), diag(zket(7,2)));
GammaD = blkdiag(pxB*(ketD*(ketD')), diag(zket(7,3)));
GammaA = blkdiag(pxB*(ketA*(ketA')), diag(zket(7,4)));
GammaR = blkdiag(pyB*(ketR*(ketR')), diag(zket(7,5)));
GammaL = blkdiag(pyB*(ketL*(ketL')), diag(zket(7,6)));
GammaMult = blkdiag(zeros(3), diag(zket(7,7)));
GammaVAC = blkdiag(diag([1,0,0]), zeros(7));
...
sqrtHV = diag([0,sqrt(pzB),sqrt(pzB),1,1,0,0,0,0,0]);
